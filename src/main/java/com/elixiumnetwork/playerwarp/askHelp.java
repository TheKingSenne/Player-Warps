...
interface askHelp
{
    static void generalHelp(final CommandSender sender, final int page) {
        ...
        if (sender.isOp() || sender.hasPermission("pwarp.staffhelp") || sender.isOp()) {
            if (page == 1) {
                ...
                sender.sendMessage(ChatColor.GOLD + " �" + ChatColor.YELLOW + " /pwarp clearoldwarps" + ChatColor.GRAY + " - " + Messages.HELP_CLEAROLDWARPS.getMessage().replaceAll("PINACTIVEDAYSP", ((PWarpPlugin)JavaPlugin.getPlugin((Class)PWarpPlugin.class)).getConfig().getInt("inactiveWarpDays") + ""));
